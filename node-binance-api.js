...
let api = function Binance(options = {}) {
  ...
  Binance.klineQueue = {};
  Binance.ohlc = {};
  Binance.depthEventsLogger = /*(type, symbol, data) => void | Promise<void> = */() => {};
  ...
  const handleSocketOpen = function (opened_callback) {
    ...
    if (Object.keys(Binance.subscriptions).length === 0) {
      Binance.socketHeartbeatInterval = setInterval(socketHeartbeat, 10000);
    }
   ...
   * @param {Function} on_close - on close callback
   * @param {string} code - code associated with the socket
   * @param {string} reason - string with the response
   * @return {undefined}
   */
  const handleSocketClose = function (reconnect, on_close, code, reason) {
    ...
    Binance.options.log(
      "handleSocketClose: closed: " +
        this.endpoint +
        (code ? " (" + code + ")" : "") +
        (reason ? " " + reason : "")
    );
    on_close && on_close(this, code, reason);
   ...
   * @param {object} closed_callback - the function to call when closed
  ...
  const subscribe = function (
    ...
    ws.isAlive = false;
    ws.on("open", () => {
    //   Binance.options.log("ws.open ", endpoint);
      handleSocketOpen.call(ws, opened_callback);
    });
    ws.on("pong", () => {
    //   Binance.options.log("ws.pong ", endpoint);
    ...
    ws.on("close", (code, reason) => {
      Binance.options.log("ws.close ", endpoint, code, reason);
      handleSocketClose.call(ws, reconnect, closed_callback, code, reason);
    });
    ws.on("message", (data) => {
      try {
  ...
  const subscribeCombined = function (
    ...
    ws.on("close", handleSocketClose.bind(ws, reconnect, closed_callback));
    ws.on("message", (data) => {
      try {
  ...
  const futuresSubscribeSingle = function (endpoint, callback, params = {}) {
    ...
    if (socksproxy !== false) {
      ...
      ws = new WebSocket(
        (Binance.options.test ? fstreamSingleTest : fstreamSingle) + endpoint,
        { agent }
      );
    } else if (httpsproxy !== false) {
      let config = url.parse(httpsproxy);
  ...
  const deliverySubscribeSingle = function (endpoint, callback, params = {}) {
    ...
    if (socksproxy !== false) {
      ...
      ws = new WebSocket(
        (Binance.options.test ? dstreamSingleTest : dstreamSingle) + endpoint,
        { agent }
      );
    } else if (httpsproxy !== false) {
      let config = url.parse(httpsproxy);
  ...
  const deliveryKlineConcat = (symbol, interval) => {
    ...
    if (typeof Binance.deliveryRealtime[symbol][interval].time === "undefined")
      return output;
  ...
  const userMarginDataHandler = (data) => {
    let type = data.e;
    // Binance.options.log("userMarginDataHandler: ",  data);
    if (type === "outboundAccountInfo") {
      // XXX: Deprecated in 2020-09-08
    } else if (type === "executionReport") {
  ...
  const depthHandler = (depth) => {
    ...
    let updateDepthCache = () => {
      Binance.depthEventsLogger('DEPTH_EVENT', symbol, depth);
    ...
    if (context.lastEventUpdateId) {
      ...
      Binance.options.logError("!!!!!!!!111", msg);
      throw new Error(msg);
    } else if (depth.u < context.snapshotUpdateId + 1) {
  ...
  return {
    ...
    withdraw: function (
      ...
      let params = { coin: asset, address, amount, walletType };
     ...
     * @return {promise or undefined} - omitting the callback returns a promise
     */
    fundingAccount: function (callback) {
      if (!callback) {
        return new Promise((resolve, reject) => {
          callback = (error, response) => {
            if (error) {
          ...
          signedRequest(sapi + "v1/asset/get-funding-asset", {}, callback, "POST");
        });
      } else {
        signedRequest(sapi + "v1/asset/get-funding-asset", {}, callback, "POST");
      }
    ...
    deliveryTerminate: function (endpoint) {
      if (Binance.options.verbose)
        Binance.options.log("Delivery WebSocket terminating:", endpoint);
    ...
    deliveryChart: async function deliveryChart(
      ...
      let handleDeliveryKlineStream = (kline) => {
        ...
        if (!Binance.deliveryMeta[symbol][interval].timestamp) {
          ...
          if (callback)
            callback(symbol, interval, deliveryKlineConcat(symbol, interval));
        }
      };
    ...
    websockets: {
        ...
        apiRequest(
          ...
          function (error, response) {
            ...
            Binance.options.execution_callback = execution_callback
              ? execution_callback
              : callback; //This change is required to listen for Orders
       ...
       * @param {function} subscribed_callback - subscription callback
       * @param {function} list_status_callback - status callback
       * @param {function} closed_callback - closed callback
       * @return {undefined}
       */
      userMarginData: function userMarginData(
        ...
        let reconnect = debounce(
          () => {
            // Binance.options.log("userMarginData.reconnect");
            if (Binance.options.reconnect)
              clearTimeout(Binance.options.listenMarginKeepAliveTimer);
              userMarginData(
                callback,
                execution_callback,
                subscribed_callback,
        ...
        apiRequest(
          ...
          function (error, response) {
            if (error) {
              Binance.options.log(
                "userMarginData v1/userDataStream error",
                error?.message
              );
            ...
            Binance.options.listenMarginKeepAliveTimer = setTimeout(function userDataKeepAlive() {
              // keepalive
              try {
                apiRequest(
                  sapi +
                  ...
                  {},
                  function (err) {
                    Binance.options.log("userMarginData.keepalive", err)
                    if (err) {
                      Binance.options.listenMarginKeepAliveTimer = setTimeout(userDataKeepAlive, 60000); // retry in 1 minute
                    }else {
                      Binance.options.listenMarginKeepAliveTimer = setTimeout(userDataKeepAlive, 60 * 30 * 1000); // 30 minute keepalive
                    }
                  },
                  "PUT"
                );
            ...
            const subscription = subscribe(
              ...
              function onSubscribe() {
                Binance.options.log("userMarginData.subscribed");
                subscribed_callback && subscribed_callback();
              },
              function onClose() {
                Binance.options.log("userMarginData.closed_callback");
                closed_callback && closed_callback();
              }
            );
            Binance.options.listenMarginSubscription = subscription;
            // if ( subscribed_callback ) subscribed_callback( subscription.endpoint );
          },
          "POST"
        );
      ...
      userMarginDataTerminate(){
        clearTimeout(Binance.options.listenMarginKeepAliveTimer)
        return terminate(Binance.options.listenMarginSubscription.endpoint, false)
      },
       ...
       * @param {function} subscribe_callback - callback function on subscribe
       * @param {function} close_callback - callback function on close
       * @return {string} the websocket endpoint
       */
      depthCache: function depthCacheFunction(
        ...
        let reconnect = debounce(
          () => {
            ...
            Binance.opetions.depthEventsLogger('DEPTH_CACHE_RECONNECT', "-", "-")
        ...
        let handleDepthStreamData = (depth) => {
          ...
          if (context.messageQueue && !context.snapshotUpdateId) {
            ...
            try {
              Binance.options.depthEventsLogger('DEPTH_STREAM', symbol, depth);
              depthHandler(depth);
            } catch (err) {
              Binance.options.logError("depthHandler2", err);
        ...
        let getSymbolDepthSnapshot = (symbol, cb) => {
          publicRequest(
            ...
            { symbol: symbol, limit: limit },
            function (error, json) {
              Binance.depthEventsLogger('DEPTH_SNAPSHOT', symbol, json)
              if (error) {
                Binance.options.logError("depthCache 3:", error);
        ...
        let updateSymbolDepthCache = (json) => {
          ...
          for (let depth of context.messageQueue) {
            ...
            try {
              Binance.options.depthEventsLogger('DEPTH_QUEUE', symbol, depth);
              depthHandler(depth);
            } catch (err) {
              Binance.options.logError("depthHandler1", err)  
              // Do nothing
            }
          }
      ...
      chart: function chart(symbols, interval, callback, limit = 500) {
        ...
        let handleKlineStreamData = (kline) => {
          ...
          if (!Binance.info[symbol][interval].timestamp) {
            if (
              typeof Binance.klineQueue[symbol][interval] !== "undefined" &&
              kline !== null
            ) {
       ...
       * @param {symbol} symbols name or false. can also be a callback
       * @param {function} callback - callback function
       * @return {string} the websocket endpoint
       */
      bookTickers: function bookTickerStream(
        ...
        // }
        let reconnect = () => {
          if (Binance.options.reconnect) bookTickerStream(symbols, callback, subscribe_callback, close_callback);
        };
        // const endpoint = symbols.map(s => `${s.toLowerCase()}@bookTicker`).join("/");
        ...
        subscription = subscribeCombined(
          ...
          function onSubscribed() {
            //on open connect
            Binance.options.log("bookTickers.subscribeCombined", "subscribed");

            subscribe_callback && subscribe_callback();
          },
          function onClose() {
            Binance.options.log("bookTickers.subscribeCombined", "on_close");
